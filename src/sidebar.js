...
TextController = withSelect( ( select ) => {
	const config = select( 'core/editor' ).getEditedPostAttribute( 'meta' )
		._unlock_protocol_config;
	return {
		_unlock_protocol_config: config,
	};
} )( TextController );

TextController = withDispatch( ( dispatch ) => {
	return {
		onMetaFieldChange: ( value ) => {
			dispatch( 'core/editor' ).editPost( {
				meta: { _unlock_protocol_config: value },
			} );
		},
	};
} )( TextController );
	...
	render: () => {
		return (
			<PluginDocumentSettingPanel
				name="unlock-protocol-config-panel"
				title="Unlock Protocol Config"
				className="unlock-protocol-config-panel"
			>
				<TextController />
				<small>
					Check{ ' ' }
					<a
						...
						target="_blank"
						href="https://docs.unlock-protocol.com/#configure-the-lock"
					>
						Unlock&apos;s documentation
					</a>{ ' ' }
					for details on the syntax to use.
				</small>
			</PluginDocumentSettingPanel>
		);
	},
