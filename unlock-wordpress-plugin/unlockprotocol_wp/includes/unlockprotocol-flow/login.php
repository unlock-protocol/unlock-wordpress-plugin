...
add_action('login_form', 'login_button');
...
function authenticate( $user = null ) 
{
	...
	try {
		$ethereum_address = validate_auth_code( $code );
		...
		*/
		$user = get_user_with_ethereum_address( $ethereum_address );
		if ( false !== $user ) {
			return $user;
		}
		...
		*/
		$email_address = get_email_address( $ethereum_address );
...
add_action('authenticate', 'authenticate');
add_filter('unlock_authenticate_user', 'authenticate');
...
* @return string
*/
function get_email_address( $ethereum_address ) 
{
	return sprintf( '%1$s@%2$s', esc_html( $ethereum_address ), esc_html( get_client_id() ) );
}
...
*/
function register( $ethereum_address ) 
{
	...
	try {
		$user_email = get_email_address( $ethereum_address );
...
add_action('unlock_protocol_register_user', 'register');
...
*/
function get_user_with_ethereum_address( $ethereum_address = '' ) 
{
	$args = array(
