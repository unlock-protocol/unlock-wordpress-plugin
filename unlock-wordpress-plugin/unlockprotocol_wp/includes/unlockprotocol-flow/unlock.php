 ...
 */
function get_locksmith_validate_url_base() 
{
 ...
 */
function get_checkout_url_base() 
{
	$settings = get_option( 'unlock_protocol_settings', array() );
 ...
 */
function has_access( $networks, $locks, $user_ethereum_address = null ) 
{
	...
	foreach($locks as $lock) {
		if (!$has_unlocked) {
			...
			$validation = validate( $url, $lock['address'], $user_ethereum_address );
 ...
 * @return float|int|\WP_Error
 */
function validate( $url, $lock_address, $user_ethereum_address = null ) 
{
 ...
 * @return string
 */
function get_checkout_url( $locks, $redirect_uri ) 
{
	$paywall_locks = array();
	$settings = get_option( 'unlock_protocol_settings', array() );
 ...
 * @return \WP_Error
 */
function validate_auth_code( $code ) 
{
	$params = apply_filters(
	...
	$response = wp_remote_post( esc_url( get_locksmith_validate_url_base() ), $args );
 ...
 * @return string
 */
function get_login_url( $redirect_uri = null ) 
{
	$login_url = add_query_arg(
		array(
			'client_id'    => get_client_id(),
			'redirect_uri' => $redirect_uri ? $redirect_uri : get_redirect_uri(),
