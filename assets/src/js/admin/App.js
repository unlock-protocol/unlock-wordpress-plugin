...
function App() {
	...
	useEffect( () => {
		apiFetch( {
			...
			.then( ( resp ) => {
				setNetworks( resp.unlock_protocol_networks );
	...
	return (
		<>
			<div className="wrap">
				<h2>{ __( 'Unlock Protocol', 'unlock-protocol' ) }</h2>
						...
						{ Object.keys( networks ).map( ( index ) => {
							return (
								<div className="single_network" key={ index }>
									<p><span>{ __( 'Network ID', 'unlock-protocol' ) }</span> : { networks[index]?.network_id }</p>
									<p><span>{ __( 'Network Name', 'unlock-protocol' ) }</span> : { networks[index]?.network_name }</p>
									<p><span>{ __( 'Network RPC Endpoint', 'unlock-protocol' ) }</span> : { networks[index]?.network_rpc_endpoint }</p>
