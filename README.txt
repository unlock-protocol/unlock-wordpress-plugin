...
Note: To allow new users to log in using their Ethereum wallet with a single click, make sure that for single site Settings > General > Anyone can register is enabled on your website and if site is multisite then Network Settings > Allow new registrations > User accounts may be registered is enabled. Your existing users can link their wallets to their WordPress user accounts regardless of this setting.
